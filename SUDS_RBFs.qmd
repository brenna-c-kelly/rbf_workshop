...
Let's compare the previous feature sets against a model also using RBFs.
...
Let's say we're happy with these predictions and are ready to interpolate. We'll need to choose points at which to extrapolate, which we can get using a grid. For each new point, we'll need to calculate the inverse distance weight from the original points in the `meuse` data.
